...
from worker.merger.ops import SemesterMergeOp
...
class Merger(MergerBase):
    ...
    def merger_fn(
        ...
        op = SemesterMergeOp(message=message)
        session.apply(op)
    ...
    def get_reducer(self):
        def reducer(storage: SessionData | None, op: BaseOp) -> SessionData:
            ...
            if isinstance(op, SemesterMergeOp):
    ...
    def create_session_storage(self) -> SessionStorage:
        return WALFileSessionStorage(
            save_dir="./sessions/saves", reducer=self.get_reducer(), op_types=[SemesterMergeOp]
        )
